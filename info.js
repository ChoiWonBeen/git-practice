...
},
{
  "이름" : "김민재",
  "이번 주 과제" : `git
  대표적인 분산형 버전 관리 시스템 -> 이거 형상 관리 시스템이라고도 합니당~
  TMI : 분산형이 있으면 당근 중앙형도 있겠지~~??
  우리는 프로그램을 만들면 만들고 땡! 이 아니다. 수정하고 추가하고 업데이트도 하고 백업도 한다. 이럴 때 버전 관리의 필요성은 당근 알겠지?!
  버전 관리를 하게 되면…
  파일이 변화하는 것을 시간에 따라 기록해 놓고 후에 특정 시간의 버전을 불러올 수 있게 된다.
  그러면 프로그램이 어떻게 바뀌었는지 추적도 할 수 있고…
  무슨 큰 일이 있으면 한 번에 전 버전으로 되돌릴수도 있다!
  그런데 규모가 큰 프로그램을 한 사람이 만드는가? 아니다. 여러 사람이 이 프로그램 개발에 참여하기 때문에 분산형 버전 관리가 필요하다.
  자, 똑같은 파일 hello.html을 가지고 A, B라는 사람이 동시에 다르게 수정해서 업데이트 하려고 하면 어떻게 해야할까? 이런 문제를 해결할 수 있다.
  즉, 분산형 버전 관리 시스템을 통해 여러 명이 동시에 병렬 개발할 수 있게 된다!
  git 사용법과 용어 정리
  merge : 말그대로 합치기, 파일들을 복사한 branch에 작업을 하다 합쳐도 되겠다 하면 merge해주는 편
  branch : 말그대로 분기
  commit : 영어로는 반납. 그렇다. 파일을 고쳐서 커밋, 저장하는 과정이다.
  checkout : 저장소에 파일을 가져옵니다. 정확히는 이전 버전 작업을 가져옵니다. check in이 commit 역할이겠죠?ㅎㅎ
  pull : 파일을 댕긴다. 원격 저장소의 데이터를 로컬 저장소에 내려 받는 걸 말할 수 있겠다.
  push : 파일을 밀어 넣는다. 평범하게 로컬 저장소의 데이터를 원격 저장소에 업로드 하는 걸 말할 수 있겠다.
  HEAD : 현재 작업 중인 branch를 말한다.
  origin : 그냥 이름이다. 따로 원격 저장소의 이름을 안정해주면 origin으로 이름이 정해진다.
  확장자
  .gitignore
  이 파일은 git 버전 관리에서 제외할 파일 목록을 지정할 수 있다.
  모든 파일들을 버전 관리 할 필요는 없으니까!!
  이거 적용하는 법은 알아둬야 할 것 같으니 따로 정리하기
  여기에다가…..
  .env
  개발자 혼자나 팀만 알아야하는 오픈소스에 올리면 안되는 값들이 있을 것이다. 그런것들을 
  .gitkeep
  .prettierc
  ㄴㅇ
  ㄴ
